...
exports.getOrderTrends = async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      dates.push(dateStr);
      orders.push(trend ? parseInt(trend.orders) : 0);
      amounts.push(trend ? parseFloat(trend.amounts) : 0);
    }
...
exports.getOrderStatusDistribution = async (req, res, next) => {
  try {
    const statusDistribution = await GameRechargeOrder.findAll({
      attributes: [
        'status',
...
exports.getCountryDistribution = async (req, res, next) => {
  try {
    const countryDistribution = await GameRechargeOrder.findAll({
      attributes: [
        'code',
      ...
      group: ['code', 'country.id'],
      order: [[sequelize.fn('COUNT', '*'), 'DESC']],
      raw: true
    });
    
    const result = countryDistribution.map(item => ({
 ...
 */
exports.getMerchantRanking = async (req, res, next) => {
  try {
    const limit = parseInt(req.query.limit) || 10;
    
    // 使用原生SQL查询来避免复杂的关联问题
    const merchantRanking = await sequelize.query(`
      SELECT 
        gro.api_key,
        COUNT(*) as orders,
        COALESCE(SUM(gro.amount), 0) as amount,
        COUNT(CASE WHEN gro.status = 2 THEN 1 END) as successOrders,
        mc.merchant_id
      FROM game_recharge_orders gro
      LEFT JOIN merchant_config mc ON gro.api_key = mc.api_key
      GROUP BY gro.api_key, mc.merchant_id
      ORDER BY COUNT(*) DESC
      LIMIT :limit
    `, {
      replacements: { limit },
    ...
    const result = merchantRanking.map(item => {
      const totalOrders = parseInt(item.orders);
      const successOrders = parseInt(item.successOrders || 0);
      const successRate = totalOrders > 0 ? (successOrders / totalOrders) * 100 : 0;
      
      return {
        merchantId: item.merchant_id || item.api_key,
        merchantName: item.merchant_id ? `商户${item.merchant_id}` : `API_KEY: ${item.api_key}`,
        orders: totalOrders,
        amount: parseFloat(item.amount || 0),
        successRate: parseFloat(successRate.toFixed(1))
      };
...
exports.getOrders = async (req, res, next) => {
  try {
    ...
    const result = rows.map(order => ({
      ...
      amount: parseFloat(order.amount),
      actualAmount: parseFloat(order.actual_amount || 0),
      status: order.status,
